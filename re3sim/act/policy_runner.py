...
"""Loading and running a trained workstation flow-BC checkpoint.
...
from dataset import ACTION_DIM, ENV_STATE_SLICE, STATE_SLICE, TASK_TAG  # noqa: E402
...
def load_checkpoint(path, device):
    """`re3sim/act/train_flow.py` checkpoint -> (policy, stats, config)."""
    ...
    # The widths alone cannot tell these tasks apart: pick-place is ALSO 16 + 25 + 7. Without
    # this tag a pick-place checkpoint loads here without a murmur and simply behaves badly,
    # which reads as a training problem rather than as the wrong file.
    assert cfg.get("task") == TASK_TAG, (
        f"{cfg} -- this checkpoint was not trained on {TASK_TAG}. Note that pick-place has "
        f"the identical 16/25/7 split, so the widths agreeing proves nothing.")
    stats = {}
