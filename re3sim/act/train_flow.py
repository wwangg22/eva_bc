#!/usr/bin/env python
"""Flow-matching BC for `Rebot-Workstation-PickPlace1-v0`.

Structurally identical to `eva_bc/clutter/act/train_flow.py`; the only difference is the
dataset module (41-D observation, `re3sim/act/dataset.py`). The pool filter is just
"successful": this task has no `episode_kind` and no per-object `outcomes`, so pick-place's
nominal / recovery / dagger pools have no analogue yet.
...
Usage
-----
    python -u re3sim/act/train_flow.py --data re3sim/expert/data/demos.hdf5 \
        --out re3sim/runs/bc_s1 --steps 60000 --seed 1
"""
...
def main() -> None:
    ...
    dataset = WorkstationDemoDataset(args.data, chunk_size=args.chunk_size)
