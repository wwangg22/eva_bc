#!/usr/bin/env python
"""Vision flow-matching chunk policy (EXP08 step 3).

FlowMatchingPolicy (act/modeling_flow.py) with the env_state token replaced by camera
...
Inputs (EXP08 section 4 no-privileged-info contract):
...
class FlowVisionTransformer(nn.Module):
    ...
    def __init__(self, config: ACTConfig):
        ...
        if config.env_state_feature:
            raise ValueError("env_state is privileged -- forbidden here (EXP08 section 4).")
