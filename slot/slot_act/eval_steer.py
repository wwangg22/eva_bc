...
from slot_act.eval_residual import load_residual_policy
...
def main() -> None:
    ...
    # EXP_STEER: score steering under the condition the frozen base fails at. Same units and
    # the same helper as eval_act.py (slot_act/noise.py), so --action-noise 0.02 here is the
    # perturbation that took the champion from 0.979 to 0.146 there -- which is precisely what
    # the arm-C gate needs to be true before any steered number is interpretable.
    parser.add_argument("--obs-noise", type=float, default=0.0, metavar="FRAC",
                        help="gaussian sensor noise, fraction of each channel's training std")
    parser.add_argument("--action-noise", type=float, default=0.0, metavar="FRAC",
                        help="gaussian actuation noise, fraction of each action channel's std")
    ...
    # Which episode this is for this env. Same reason as eval_act.py: the first episode a
    # process runs scores measurably higher (+12.9 pts for the expert, 4.3 sigma), so the gate
    # must compare later-cohort against later-cohort or it compares warm-start bias.
    ...
    while len(records) < args.episodes:
        ...
        core.flush_check(u)
        # Perturb only what the CONTROLLER sees and commands. `obs` itself stays clean, so the
        # steering observation built at the window boundary above is clean too -- the same
        # asymmetry steer_wrapper.py trains under, and it must match or the eval is not
        # measuring the policy that was trained. (The easier version: the steerer sees a clean
        # view of a corrupted rollout. The harder version is noisy steering obs as well.)
        ctrl_in = obs if obs_sigma is None else obs + args.obs_noise * obs_sigma * torch.randn_like(obs)
        action = core.controller.act(ctrl_in)
        if act_sigma is not None:
            # after the controller, so the noise corrupts the COMMAND rather than the plan.
            action = action + args.action_noise * act_sigma * torch.randn_like(action)
    ...
    result = {
        ...
        # Compare arms on `success_rate_later` -- see eval_act.py for why.
        ...
        # Recorded for the same reason eval_act.py records them: a run under noise is otherwise
        # byte-indistinguishable from a clean one, and the gate turns on exactly this field.
        "config": {"num_envs": n, "obs_noise": args.obs_noise,
                   "action_noise": args.action_noise, "flush": bool(args.flush),
    ...
    print(f"[eval_steer]   first-episode {_f(result['success_rate_first_episode'])} "
